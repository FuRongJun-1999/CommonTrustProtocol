...
DOMAIN_KEYWORDS = {
    "compiler": ["编译", "词法", "语法", "VM", "字节码", "序列化", "调试",
                 ...
                 "三元表达式", "复合赋值", "位运算", "标识符解析", "操作符解析", "函数签名", "栈操作", "算术执行", "比较执行", "信任检查", "信息差追踪", "条件空间类型", "条件断点", "调用计数", "覆盖率", "窥孔优化", "指令融合", "循环不变式", "字典字面量", "元组解析", "转义序列", "条件空间符号类型", "布尔字面量", "空值字面量", "行号跟踪", "关键字识别", "常量池", "语句分隔", "异常处理", "表达式树", "栈深度分析", "基本块", "支配树", "中间表示", "循环检测", "指令调度", "寄存器溢出", "数组操作", "污点分析", "边界检查消除"],
    "pylang": ["Python", "表达式", "闭包", "闭包机制", "控制流", "作用域", "栈机",
               ...
               "归约", "字符串拆分", "字符串替换", "字符串判断", "数学函数", "数值舍入", "数值统计", "默认参数", "关键字参数", "多行字符串", "文件读取", "性能计时", "环境查询", "链表", "进制转换", "异常链", "二叉树", "迭代工具", "字典合并", "最小堆", "函数缓存", "异步生成器", "切片操作", "矩阵运算", "类装饰器", "默认字典", "排列组合", "二分查找", "字典推导", "异步队列", "模板渲染", "有序字典", "随机采样", "字节编解码", "字符串哈希", "跳表", "时间格式化", "字符串对齐", "顺序去重", "滑动均值", "行程压缩", "位掩码", "众数统计", "分位数", "文本分词", "笛卡尔积"],
    "graph": ["图数据库", "图遍历", "图查询", "条件路由图", "遍历", "路径",
              ...
              "信任传播", "信息差收敛", "布隆过滤", "租户隔离", "增量备份", "一致性快照", "一致性哈希", "邻居查询", "路径过滤", "三角形计数", "导出子图", "时间窗口", "边活跃度", "模式路径", "节点标签", "模糊匹配", "条件回溯", "信任聚合", "审计日志", "审计记录", "强连通分量", "二分匹配", "可达性判定", "传递闭包", "图着色", "最小割", "聚类系数", "介数中心性", "边索引", "割点", "独立集", "桥检测", "游标遍历", "路径规划", "节点大小", "最大团", "旅行商", "标签约束"],
    "os": ["进程", "调度", "内存", "文件系统", "路径解析", "管道", "IPC",
           ...
           "进程生命周期", "磁盘调度", "写时复制快照", "磨损均衡", "强制访问控制", "系统调用过滤", "加密文件系统", "服务管理", "日志轮转", "定时任务", "配置管理", "权限提升", "环境变量", "网络接口", "设备驱动", "电源管理", "文件压缩", "存储池", "文件版本", "条件变量", "自旋锁", "时间片轮转", "优先级继承", "内存池", "稀疏文件", "碎片整理", "段式管理", "时钟节拍"],
    "browser": ["浏览器", "HTTP", "响应解析", "DOM", "HTML", "CSS", "选择器",
                ...
                "拖放", "资源完整性", "媒体播放", "地理位置", "全屏模式", "预加载", "请求合并", "资源优先级", "权限API", "CSP报告", "支付请求", "响应式断点", "离线队列", "会话恢复", "CORS", "跨域资源共享", "文本排版", "剪贴板", "沙箱隔离", "滚动容器", "在线状态", "命中测试", "标签页通信", "页面可见性", "像素光栅化", "触控手势", "设备方向", "渐变填充", "网络信息", "字体加载", "颜色转换", "振动反馈", "混合模式", "边框圆角", "屏幕方向", "空闲调度", "摄像头", "蓝牙扫描", "传感器", "语音合成", "语音识别", "扫码", "画中画", "屏幕捕获", "文件系统访问", "File System", "网页共享", "唤醒锁", "窗口管理", "翻译", "语言检测", "文本摘要", "联系人", "形状检测", "存储配额", "内置聊天", "图像生成", "提示词"],
    "net": ["网络", "TCP", "UDP", "IP分片", "握手", "校验和", "广播",
            ...
            "策略路由", "多径传输", "访问令牌", "压缩传输", "会话亲和", "链路加密", "流量镜像", "网络切片", "分块传输", "重定向", "内容协商", "路径MTU", "端口扫描", "会话超时", "消息路由", "API限流", "数据序列化", "DHCP", "租约", "ARP", "地址解析", "ICMP", "Ping", "广播风暴", "心跳保活", "报文重排序", "CSMA", "路由收敛", "链路预算", "RSSI", "NTP同步", "报文分片", "前向纠错", "尽力交付", "带宽时延积", "多宿主", "RTT平滑", "路由汇聚", "序列号回绕", "漏桶限流", "报文调度", "链路利用率", "带宽分配", "连接迁移", "重传统计"],
}
