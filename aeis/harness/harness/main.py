...
def main(argv=None):
    ...
    log(f"灵枢原生运行时 v1.1 启动（语音={not no_voice} 调度={not no_sched} "
    ...
    # 2.1 插件管理器（MCP Client）+ 子智能体编排器（v1.1 扩展）
    plugin_manager = None
    supervisor = None
    if not no_plugins:
    ...
    def handle_input(text: str):
        ...
        if supervisor is not None and any(
            ...
            try:
                ...
                agg = supervisor.aggregate([task.task_id])
