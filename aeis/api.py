...
class Agent:
    ...
    def __init__(self, identity: str = "智能体",
        ...
        # v1.30（规范即机制）：每次初始化强制加载「回答规范锚点」——
        # 条件路由图 + 自然语言编译作为自我层（SELF/ANCHOR），不可遗忘。
        try:
            import sys as _s
            import os as _os
            _wdir = _os.path.join(_os.path.dirname(_os.path.dirname(
                _os.path.abspath(__file__))), "wisdom")
            if _wdir not in _s.path:
                _s.path.insert(0, _wdir)
            from answer_spec_anchor import write_answer_spec_anchors
            write_answer_spec_anchors(self)
        except Exception:
            pass  # 锚点写入失败不阻塞 Agent 创建（幂等，下次再写）
