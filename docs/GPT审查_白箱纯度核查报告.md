...
1. **semantic_induced 候选仅保留显式因果链**（当前 `has_structural_pattern`/偏好权重仍可引入非因果候选——白箱纯度上限）
2. **被拒路径自动条件化**：误差 → 自动触发条件空间更新（当前半自动：飞轮/蒸馏驱动触发 causal_discover）
3. **条件节点化入主库**：把 compose_engine 的条件化单元格式引入语义时空图（条件节点→适用关系→知识节点）——最大工程，对接白箱自举第三阶段
4. **推导过程显式记录**：蒸馏/归纳保存「证据→聚类→模式」的中介逻辑（当前存证据摘要，未存聚类算法参数）
