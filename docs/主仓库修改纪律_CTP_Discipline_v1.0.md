# 主仓库修改纪律（CTP Discipline v1.1 · 2026-09-05，v1.0 2026-08-27）
...
### R7 工具面有界 · 补全优先（邓巴数原则，2026-09-05 荣裁定）
- MCP 工具面对已有功能**补全，不盲目扩容**——新需求先查既有工具可否补全/组合实现；**可组合时不新增工具**（新增须论证「现有工具为何不可组合」，引用实测盘点清单）。
- 工具设计四原则：**简单**（一个工具一个职责）、**可组合**（工具互相搭配成流程）、**专业化**（并入既有族用 action 分发，如 world_model / grill_node 范例）、**模块化**（族内演进，历史工具下沉不删除）。
- 工具面数量有界（参考邓巴数：人维持稳定关系的认知上限 ~150；工具注入与模型选择可靠性同样有认知上限）。当前全量注册 86（2026-09-05 实测）；对外暴露走 `AEIS_MCP_TOOLS` 白名单（§AGENTS.md 工具集裁剪），**工具面增长须设计者裁定**。
...
- [ ] README 数字已同步实测值（R4）
- [ ] 运行时产物未入库（R3）
- [ ] **新增 MCP 工具时**：KCCS 四要素工具描述齐全；新工具单测通过；工具数实测并同步 README/AGENTS.md 速查/白名单建议三处（R4/R7）
...
- **v1.1（2026-09-05）**：新增 R7 工具面有界·补全优先（邓巴数原则）；发布清单增补新工具条款。
  案例一：grill 访谈工具族落地——发布前仅跑部分测试被设计者纠正，暴露「新工具发布检查」无显式条款；
  同日工作纪律第 13 条「访谈澄清」并入认知图（工具+纪律配套：方法论进条件卡，机制进 MCP）。
- v1.0（2026-08-27）：初版（副本污染事故后立：八步闭环 + 六红线 + 同步矩阵 + 发布清单）。
