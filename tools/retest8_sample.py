# -*- coding: utf-8 -*-
"""retest8_sample.py · 第8次随机复测锚定：随机抽20题（固定种子）逐题验证。"""
...
def main():
    ...
    qs = bank["questions"]
    # 老题锚定：排除最近九批（通识拓展495~503）新增，验证早期入库知识稳定性
    recent = {q["id"] for q in qs if q.get("source", "").startswith(("通识拓展49", "通识拓展50",))}
    ...
    print("== 第8次随机复测锚定（种子20260908，池%d题抽20）==" % len(pool))
    hits = []
    for q in sample:
