...
def classify(drift: list) -> dict:
    """分类：弱兜底（可吸收——实现返回默认值）/ 其他。"""
    absorbable, manual = [], []
    for d in drift:
        got = d.get("got", "")
        cond = d.get("cond", "")
        # 弱兜底：返回具体值（非 None/False/异常）——实现有默认语义
        if got not in (None, 'None', 'False', '') and \
...
def _in_string_edit(text: str, cond: str, got: str) -> str:
    ...
    note = f"（返回 {str(got)[:30]} 兜底——不拒绝，弱契约）"
