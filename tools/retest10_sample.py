# -*- coding: utf-8 -*-
"""retest10_sample.py · 第10次随机复测锚定：随机抽20题（固定种子）逐题验证。"""
...
def main():
    ...
    qs = bank["questions"]
    # 老题锚定：排除最近三十九批（通识拓展495~533）新增，验证早期入库知识稳定性
    recent = {q["id"] for q in qs if q.get("source", "").startswith(
        ("通识拓展49", "通识拓展50", "通识拓展51", "通识拓展52", "通识拓展53",))}
    ...
    print("== 第10次随机复测锚定（种子20260910，池%d题抽20）==" % len(pool))
    hits = []
    for q in sample:
