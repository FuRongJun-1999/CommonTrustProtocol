...
test('插件激活：lingxu_* 工具注册进 ctx.tools', async () => {
  const dir = mkdtempSync(join(tmpdir(), 'lingxu-host-'))
  const host = await mountHost(join(dir, 'host.db'))
  try {
    const schemas = host.root.tools.schemas()
    const names = schemas.map((s) => s.name)
    assert.ok(names.includes('lingxu_remember'), `应注册 lingxu_remember（实际有: ${names.slice(0, 5).join(', ')}…）`)
    assert.ok(names.includes('lingxu_recall'), '应注册 lingxu_recall')
    // core 集合精选 12 个，不应注册全部（如 designer_decide 不在 core）
    assert.ok(!names.includes('lingxu_designer_decide'), 'core 集合不应包含 designer_decide')
...
test('插件卸载：工具注销且进程退出', async () => {
  ...
  const host = await mountHost(join(dir, 'host.db'))
  try {
    assert.ok(host.root.tools.schemas().some((s) => s.name === 'lingxu_remember'), '激活后应有工具')
    host.disposePlugin()
    await new Promise((resolve) => setTimeout(resolve, 300))
    assert.ok(
      !host.root.tools.schemas().some((s) => s.name.startsWith('lingxu_')),
      '卸载后 lingxu_* 工具应注销',
    )
  } finally {
    host.disposeAll()
