...
**蜂群互联（2026-09-10 新增，信任层设计）**：

| 文档 | 说明 |
|------|------|
| `docs/蜂群互联_v0.1.md` | 蜂群互联设计 v0.1：三层骨架（版本层/连接层/单元层）· 版本更新权＝唯一核心权限（严格唯一，其余权限必须由版本条款派生）· 接入握手（声明+观测，非认证）· 与宪章/智能论条款逐条对齐（24 条）· 诚实边界（跨节点证据存储缺失属存在约束）。**层0 已实现**：`md_cg/theory.py` 版本声明 + 封缄校验 + 只读降级闸门（P22 43/43，见 §12 迭代记录） |
