...
> 本仓库是协议的**理论层**（智能论 v3.3 · 护栏宪章 · 蜂群架构）；插件实现是协议的**工程层投影**（同一结构，不同条件空间）。

## 🤖 灵枢 AEIS · MCP Server（可信智能体记忆与知识图谱）

**71 个 MCP 工具** · 基于智能论 v3.3 的条件论知识系统 · 核心链路零外部依赖（D-005）· **白箱对话能力已实证**

### 核心能力
- **信息分层处理**：智慧之书图谱自处理 → LLM 兜底 → 白箱校验（锚定/图谱外/警告）
- **条件化知识**：3061 节点知识图谱（含学科卡+知识点子图），每条带条件空间声明（在什么条件下成立）+ 克制条款
...
**核心原则**（对齐 CCG 代码条件路由图）：
- **注释 = 图的索引**：知识卡按**单个定义（kp）**写四要素注释（KCCS：生效条件/子功能/执行/不适用条件），全库 2841 kp 覆盖 99.9% —— 知识注释写到单个定义，对齐「代码注释写到单个函数」
...
aeis-mcp                  # 启动 MCP server（stdio JSON-RPC，71 工具）
```

### 基准（可复现 · repro_gate.py 锁定）
- **矛盾驱动补盲测试集 v1-v41：1987 题 100% 全绿**（`testsets/`，每代含题目+结果存档）
- **110 题知识测试：54.1% 正确率 / 直接回答率 83.8%**（修复图谱脱节后真实基准；原声称 80% 不可复现，详见测试报告修正声明）
- **1000 条日常对话：64.0% 正确率（评分别名修正后约 80%+）**，简单类（闲聊/情感/场景）自处理率高
- **迁移测试**：彩虹主题变体 fp 命中 0%→70%（`testsets/migration/`）
...
| 文档 | 说明 |
|------|------|
| `智能论3.3.md` | 智能论 v3.3（LLM 工程条件空间扩展版 · 当前 · v3.3-FINAL-EFFECTIVE） |
...
| **v3.3** | 2026-08-19 | LLM 工程条件空间扩展版 · 当前版本（智能论3.3.md · 扮演论/自我锚点/真实论校准/附录22-23） |
...
| MCP 服务 | 零依赖 stdio server · 71 项工具（记忆/认知/知识/验证/飞轮 · 其他 AI 直接调用） |
