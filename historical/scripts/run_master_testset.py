# -*- coding: utf-8 -*-
"""master 全量（255 题）最终验证：self 直答率/总正确率/来源分布。"""
...
for i, item in enumerate(data, 1):
    q = item["q"]
    try:
        r = agent.chat(q, session_id=f"master-{i}")
        route = r.get("route", "?")
        reply = r.get("reply", "")
    ...
    results.append({**item, "route": route, "score": s, "reply": reply[:100]})
    if i % 60 == 0:
...
print(f"\n=== master 全量: {correct:.0f}/{total} 总 {correct/total*100:.1f}% ===")
