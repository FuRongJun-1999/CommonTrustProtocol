# -*- coding: utf-8 -*-
"""矛盾测试集 v2 基线：84 题正反合链跑白箱 → 按域/阶段定位断裂。"""
...
data = json.load(open(r"D:\Program Files\2_ai\knowledge-base\conflict_testset_v2.json", encoding="utf-8"))["items"]
...
for i, item in enumerate(data, 1):
    q = item["q"]
    try:
        r = agent.chat(q, session_id=f"cv2-{i}")
        route = r.get("route", "?")
        reply = r.get("reply", "")
...
print(f"\n=== v2 基线: {ok}/{total} 合格 ({ok/total*100:.0f}%) ===")
...
for x in results:
    if x["bad"]:
        print(f"  [{x['domain']}/{x['stage']}] {x['q'][:36]} | {x['route']}")

with open(r"D:\Program Files\2_ai\knowledge-base\conflict_testset_v2_results.json", "w", encoding="utf-8") as f:
    json.dump({"total": total, "ok": ok, "results": results}, f, ensure_ascii=False, indent=1)
print("\n已存 conflict_testset_v2_results.json")
agent.close()
