# -*- coding: utf-8 -*-
"""全量回归 v44-v60（c12 后无损验证）：逐集 agent.chat 判断 bad，统计每集 ok/n"""
...
print(f"\n=== 全量回归 v44-v60: {all_ok}/{all_total} ({all_ok/all_total*100:.1f}%) ===")
for k, v in summary.items():
    print(f"  {k}: {v}")
with open(os.path.join(KB, "regress_c12_v44_v60.json"), "w", encoding="utf-8") as f:
    json.dump({"total": all_total, "ok": all_ok, "rate": all_ok/all_total,
               "summary": summary}, f, ensure_ascii=False, indent=1)
print("已存 regress_c12_v44_v60.json")
agent.close()
