...
check('③c os 链：软中断 → 工作窃取 → 模块加载', c2['ok'] and len(c2['chain']) == 3,
      ' → '.join(c2['chain']))
check('③d 组装 verifier 闭环：链上单元全过 + 组装语法通过',
      c['verify']['all_ok'] and c['verify']['syntax_ok']
      and all(x['cached'] for x in c['verify']['units']),
      f"all_ok={c['verify']['all_ok']} syntax={c['verify']['syntax_ok']}")
