...
data = json.load(open(r"D:\Program Files\2_ai\knowledge-base\conflict_testset_v37b.json", encoding="utf-8"))["items"]
...
for i, item in enumerate(data, 1):
    q = item["q"]
    try:
        r = agent.chat(q, session_id=f"cv37b-{i}")
        route = r.get("route", "?")
        reply = r.get("reply", "")
...
print(f"\n=== v37b: {ok}/{total} ({ok/total*100:.0f}%) ===")
print("route:", dict(Counter(x["route"] for x in results)))
print("=== 断裂清单 ===")
for x in results:
    if x["bad"]:
        print(f"  [{x['domain']}/{x['stage']}] {x['q'][:34]} | {x['route']} | {(x['reply'] or '')[:40]}")
with open(r"D:\Program Files\2_ai\knowledge-base\conflict_testset_v37b_results.json", "w", encoding="utf-8") as f:
    json.dump({"total": total, "ok": ok, "results": results}, f, ensure_ascii=False, indent=1)
print("已存 conflict_testset_v37b_results.json")
agent.close()
