...
data = json.load(open(r"D:\Program Files\2_ai\knowledge-base\conflict_testset_v15.json", encoding="utf-8"))["items"]
...
for i, item in enumerate(data, 1):
    q = item["q"]
    try:
        r = agent.chat(q, session_id=f"cv15-{i}")
        route = r.get("route", "?")
        reply = r.get("reply", "")
...
print(f"\n=== v15 基线: {ok}/{total} ({ok/total*100:.0f}%) ===")
...
with open(r"D:\Program Files\2_ai\knowledge-base\conflict_testset_v15_results.json", "w", encoding="utf-8") as f:
    json.dump({"total": total, "ok": ok, "results": results}, f, ensure_ascii=False, indent=1)
print("\n已存 conflict_testset_v15_results.json")
agent.close()
