# -*- coding: utf-8 -*-
"""新测试集 v1 验证：跑 72 题 → self 直答率 / 总正确率基线。

新题库尚未补卡——此跑是「盲区探测基线」：看三来源哪些已能被白箱直答，
哪些仍需补卡（self 直答率 <95% 的区域 = 下一轮补卡目标）。
"""
...
for i, item in enumerate(data, 1):
    q = item["q"]
    try:
        r = agent.chat(q, session_id=f"newset-{i}")
        route = r.get("route", "?")
        reply = r.get("reply", "")
...
print(f"\n=== 新测试集基线: {correct:.0f}/{total} 总 {correct/total*100:.1f}% ===")
