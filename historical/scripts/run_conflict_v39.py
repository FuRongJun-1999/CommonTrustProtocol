...
data = json.load(open(r"D:\Program Files\2_ai\knowledge-base\conflict_testset_v39.json", encoding="utf-8"))["items"]
...
for i, item in enumerate(data, 1):
    q = item["q"]
    try:
        r = agent.chat(q, session_id=f"cv39-{i}")
        route = r.get("route", "?")
        reply = r.get("reply", "")
